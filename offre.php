...
<head>
    ...
    <link rel="stylesheet" href="maincss/offre.css?v=<?php echo time(); ?>">
...
if (isset($_GET['ok'])) {
  ...
  $sqlm="insert into demande VALUES('','{$_GET['message']}','{$_GET['idoffre']}','{$idEmp}')";
  $reqm=mysqli_query($con,$sqlm);
  $deja=1;
}}
   if (isset($_GET['idoffre'])) {
     $sql="SELECT idOffre, o.idEntreprise identreprise,poste,qualification,o.description description, dateDebut, dateFin, vue, Salaire, reseauSocial,siegeSocial,nomVille,avatar,domaineActivite, libelle, typeContrat FROM offres o JOIN entreprises join secteur join ville where idOffre={$_GET['idoffre']} GROUP BY idOffre";
   }
...
echo "
    <div class='cvrbg-jb'>
        <div class='cvr-lr container-fluid ''> 
           <h1 class='cvr-h '>{$offre['poste']}</h1>
           <h4 class='first-title ' > {$offre['reseauSocial']}</h4>
             <h4 class='first-title ' style='font-style: italic;'> {$offre['dateDebut']} | {$offre['dateFin']}</h4>    
       </div>
...
<?php
               echo "<div class='jobas-body '>
                	<h4 class='mb-3'>{$offre['poste']}</h4>
                 <p>{$offre['description']}</p>
                <h5>Profil recherché :</h5>
                <p>{$offre['qualification']}</p>
                <div class=infocontainer>
               <h5 class='mb-3'>Autres information :</h5>
               <div class='row' >
                 <div class=col-sm-6>
                    <p>La ville d'offre : <span class=info>{$offre['nomVille']}</span></p>
                    <p>Le secteur d'offre : <span class=info>{$offre['libelle']}</span></p>
                  </div>
               
               <div class=col-sm-6>
                    <p>Le type de contrat : <span class=info>{$offre['typeContrat']}</span></p>
                    <p>Le salaire de poste : <span class=info>{$offre['Salaire']} DH</span></p>
               </div>
    ...
    ?>
         ...
         <div class="col-md-3   right-side"   >
            ...
            <div class=" cv-content" >
             ...
             echo "
                <div class='card-body'>
                  <img src='{$offre['avatar']}' >
                  <h6>{$offre['reseauSocial']}</h6>
                  <p class='cd-txt'>
                    <span class='text-primary'>- </span>{$offre['siegeSocial']} <br>
                   <span class='text-primary'>- </span>{$offre['domaineActivite']} <br>
                  <span class='text-primary'>- </span><a href='profileEnt.php?q={$offre['identreprise']} '>Çavoir tous sur l'entreprise</a>
             ...
             ?>
                  ...
                  <div class="postuler">
                    ...
                    <?php
                    if ($deja==1) {
                      echo"<h5>félicitaion !</h5><h5>tu à postulé a cet offre avec succée</h5>";
                    }
                    else{
                      echo"<h5>Intéressé·e ?</h5><h5> Postulez dès maintenant</h5>
                    
                  <form>
                      <div class='input-group mt-5'>
  
                         <textarea class='form-control' name='message' rows='4'  aria-label='With textarea'></textarea>
                      </div>
                      <input type='hidden'  name='idoffre' value='{$_GET['idoffre']}'>
                      <div class='input-group mt-3'>
                      <input type='submit'name='ok' class='form-control creer' value='Je postule'>
                    </div>
