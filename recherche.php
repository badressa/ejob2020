...
<head>
    ...
    <link rel="stylesheet" href="maincss/recher.css?v=<?php echo time(); ?>">
...
<div class="full-background first-contai" style="background-color: #f2f2f4;">
    ...
    <div class='row ' >
         <div class=" col-md-9 jobs" >
              ...
              <!---selection des  offres recherches----->

                <div class="jobas-body">
                	<?php
               if (isset($_GET['ok'])) {
                 if (empty($_GET['ville'])) {
                  $sql2="SELECT * FROM offres WHERE poste  like '%{$_GET['emploi']}%' ORDER BY dateDebut desc ";
                                            }

                 else {
                  $sql2="SELECT * FROM offres WHERE poste  like '%{$_GET['emploi']}%' AND idVille='{$_GET['ville']}' ORDER BY dateDebut desc ";
                      }
               }
               if(isset($_GET['idsecteur'])) {
                $sql2="SELECT * FROM offres WHERE idSecteur ='{$_GET['idsecteur']}'  ORDER BY dateDebut desc ";
                }
                $req2=mysqli_query($con,$sql2);
                     if (mysqli_num_rows($req2)==0) {
                       echo "<p class=vide>Il ya aucun resultat pour votre recherche !</p>";
                     }
                     else{
